...
type Screen = 'menu' | 'transitioning' | 'setup' | 'playing';
...
export default function App() {
  ...
  const setupRippleRef = useRef<{ x: number; y: number }>({ x: 0, y: 0 });

  function handleVsComputer(e: React.MouseEvent<HTMLButtonElement>) {
    ...
    setTimeout(() => setScreen('setup'), 680);
  }

  if (screen === 'playing') {
    return <GameBoard opponents={opponentsRef.current} onExit={() => setScreen('menu')} />;
  }
    ...
    return (
      <GameShell topbar={
        <GameTopbar title="UNO" actions={
          <GameButton size="sm" variant="ghost" onClick={() => setScreen('menu')}>← Back</GameButton>
        } />
      }>
        <div
          className="relative flex flex-col items-center justify-center h-full overflow-hidden bg-[#0d0520]"
          style={{ gap: 'clamp(12px, 3vh, 40px)', padding: '12px 16px' }}
        >
          {/* Ripple transition overlay */}
          {setupTransitioning && (
            <div
              className="fixed w-10 h-10 rounded-full z-[200] pointer-events-none"
              style={{
                background: '#166534',
                left: setupRippleRef.current.x - 20,
                top: setupRippleRef.current.y - 20,
                animation: 'ripple-expand 0.68s cubic-bezier(0.4,0,0.2,1) forwards',
              }}
            />
          )}
          ...
          {[
            ...
            <div key={i} className="absolute w-[260px] h-[260px] rounded-full pointer-events-none" style={{
          ...
          {SHAPES.map((s, i) => (
            <div key={i} className="absolute rounded-lg pointer-events-none" style={{
              width: s.w,
              height: s.h,
          ...
          <div className="text-center relative" style={{ animation: 'fade-up 0.5s ease both', animationDelay: '0.05s' }}>
            <p className="text-white/40 text-[12px] font-sans uppercase tracking-[0.12em] m-0 mb-[6px]">
              vs Computer
            </p>
          ...
          <div
            ...
            style={{ gap: 'clamp(6px, 1.5vw, 15px)', animation: 'fade-up 0.5s ease both', animationDelay: '0.1s' }}
          >
            {/* Top seat */}
            ...
            <div className="flex items-center" style={{ gap: 'clamp(6px, 1.5vw, 15px)' }}>
              ...
              <div
                ...
                style={{
                  width: 'clamp(130px, 34vw, 222px)',
                  height: 'clamp(86px, 20vw, 144px)',
              ...
              >
                <div
                  ...
                  style={{ width: '65%', height: '62%', borderRadius: 'clamp(8px, 2vw, 18px)' }}
                />
              </div>
            ...
            {/* You (bottom) */}
            <div
              className="flex items-center justify-center text-white font-bold font-sans border-2 border-white/20 bg-white/8"
              style={{
                width: 'clamp(72px, 20vw, 120px)',
          ...
          <div className="flex gap-[14px] relative" style={{ animation: 'fade-up 0.5s ease both', animationDelay: '0.15s' }}>
            {(['Alpha', 'Beta', 'Gamma'] as SeatType[]).map(t => (
              <div key={t} className="flex items-center gap-[5px]">
                ...
                <span className="text-white/50 text-[11px] font-sans">
                  {t} · {t === 'Alpha' ? 'Hard' : t === 'Beta' ? 'Medium' : 'Easy'}
                </span>
              </div>
            ))}
          </div>
          ...
          <div className="relative" style={{ animation: 'fade-up 0.5s ease both', animationDelay: '0.2s' }}>
            <button
              onClick={e => {
                const rect = e.currentTarget.getBoundingClientRect();
                opponentsRef.current = resolveOpponents(topSeat, leftSeat, rightSeat);
                setupRippleRef.current = { x: rect.left + rect.width / 2, y: rect.top + rect.height / 2 };
                setSetupTransitioning(true);
                setTimeout(() => { setSetupTransitioning(false); setScreen('playing'); }, 680);
              }}
              ...
              onMouseEnter={e => {
                ...
                e.currentTarget.style.boxShadow = '0 8px 28px rgba(220,38,38,0.6)';
              }}
              onMouseLeave={e => {
          ...
          <a
            ...
            className="text-white/25 text-[11px] font-sans no-underline relative"
  ...
  // Menu (includes 'transitioning' state for ripple overlay)
  return (
    <GameShell topbar={<GameTopbar title="UNO" />}>
      <div
        className="relative flex flex-col items-center justify-center h-full gap-7 p-6 overflow-hidden"
        style={{
          background: `
            radial-gradient(ellipse 110% 45% at 50% 0%, rgba(220,38,38,0.18) 0%, transparent 60%),
            radial-gradient(ellipse 55% 35% at 10% 90%, rgba(37,99,235,0.12) 0%, transparent 55%),
            radial-gradient(ellipse 55% 35% at 90% 90%, rgba(124,58,237,0.12) 0%, transparent 55%),
            linear-gradient(170deg, #0a1628 0%, #060e1c 100%)
          `.replace(/\s+/g, ' '),
        }}
      >
        {/* Ripple transition overlay */}
        {screen === 'transitioning' && (
          <div
            ...
            style={{
              background: '#0d0520',
              left: rippleRef.current.x - 20,
              top: rippleRef.current.y - 20,
              animation: 'ripple-expand 0.68s cubic-bezier(0.4,0,0.2,1) forwards',
            }}
          />
        )}
        ...
        <div className="text-center" style={{ animation: 'menu-card-in 0.5s ease both' }}>
          <div
            ...
            style={{
              ...
              width: 148,
              height: 100,
              boxShadow: '0 8px 32px rgba(185,28,28,0.5), inset 0 2px 0 rgba(255,255,255,0.15)',
              transform: logoHovered ? 'rotate(-10deg) scale(1.12)' : 'rotate(-10deg)',
              transition: 'transform 0.35s cubic-bezier(0.34,1.56,0.64,1)',
            }}
          >
            {glazeKey > 0 && (
              <div key={glazeKey} className="absolute pointer-events-none" style={{
                top: '-20%',
                left: 0,
                width: 72,
                height: '140%',
                ...
                animation: 'logo-glaze 0.55s ease forwards',
              }} />
            )}
          ...
          {MODES.map((mode, i) => (
            <button
              ...
              style={{
                ...
                background: mode.bg,
                border: 'none',
                cursor: mode.locked ? 'default' : 'pointer',
                padding: '24px 12px 20px',
                opacity: mode.locked ? 0.5 : 1,
                filter: mode.locked ? 'grayscale(1)' : 'none',
                boxShadow: mode.locked ? 'none' : '0 6px 24px rgba(0,0,0,0.3)',
                transition: 'transform 0.18s cubic-bezier(0.34,1.56,0.64,1), box-shadow 0.18s ease',
                animation: `${mode.locked ? 'menu-card-in-locked' : 'menu-card-in'} 0.55s cubic-bezier(0.34,1.56,0.64,1) both`,
              ...
              onMouseLeave={e => {
                e.currentTarget.style.transform = 'none';
                e.currentTarget.style.boxShadow = mode.locked ? 'none' : '0 6px 24px rgba(0,0,0,0.3)';
              }}
            ...
            >
              ...
              <div className="text-center relative">
                <p className="text-white font-serif font-bold text-[15px] m-0 mb-[5px] leading-snug">
                  {mode.label}
                </p>
                <p className="text-white/60 font-sans text-[11px] m-0">
                  {mode.sub}
                </p>
              </div>
            </button>
        ...
        <a
          href="https://freeappstore.online"
          className="text-white/30 text-[11px] font-sans no-underline"
          style={{ animation: 'menu-card-in 0.5s ease both', animationDelay: '0.35s' }}
        >
          FreeGameStore
        </a>
      </div>
